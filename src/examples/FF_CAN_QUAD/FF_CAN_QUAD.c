/**
 * @FF_CAN.c
 * Freefly Systems CAN bus for use on Alta Quad
 *
 * @author Selim Ozel <selim.ozel@freeflysystems.com>
 * @author Cory Schwharzmiller <cory@freeflysystems.com>
 * @author Jeremy Zanzig <jeremy@freeflysystems.com>
 */

//-------------------------------------------------------------------------------------
// Includes
#include "examples/FF_CAN_QUAD/FF_CAN_QUAD.h"
...
void FF_CAN(void)
{
	...
	{
		PX4_ERR("FF_CAN - FAILED TO JUMP CAN MODULES");
		return;
	}
...
int FF_CAN_Bootloader_Jump(void)
{
	...
	ret = FF_CAN_BootloaderJumpId(9, 0x4, 0x08008000);	// jump osd mcu
	if (ret != 0){
		PX4_ERR("OSD MCU jump to application failed! - aborting bootloader jump!");
		return 1;	// stop and return if there is an error!
	}
	
	ret = FF_CAN_BootloaderJumpId(10, 0x5, 0x08008000);	// jump power module
	if (ret != 0){
...
{
	// On Quad, we aren't controlling the motors, but to prevent recompile we still need this hook for the arming sequence. 
	// Just always return success.	_motorArmAck_BitFlags = 0;
...
void FF_CAN_WriteTask(void)
{
	...
	int batteryStatus = orb_subscribe(ORB_ID(battery_status));
	struct battery_status_s batteryStatusData;

	// Only run at 5Hz for now
	orb_set_interval(batteryStatus, 200);
	...
	while(!_writeThreadShouldExit)
	{
		...
		// Wait for up to 500ms for data while waiting for uorb data
		int myRet = px4_poll(&pollfd, 1, 500);
		...
		if (pollfd.revents & POLLIN) {
			...
			orb_copy(ORB_ID(battery_status), batteryStatus, &batteryStatusData);

		// TODO - Do something with the poll data, most likely send lights command
...
int FF_CAN_QUAD_main(int argc, char *argv[])
{
